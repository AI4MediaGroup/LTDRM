...
class VideoRecord(object):
    def __init__(self, video_path, landmark_file_path, start_frame, end_frame, label, reverse=False):
...
class VideoDataSet(data.Dataset):
    ...
    def _parse_list(self):
        # [video_path, align_file_path, start_frame, end_frame, class_idx]
        tmp = []   
        original_video_numbers = 0
        for x in open(self.list_file):
            elements = x.strip().split(self.separator)
            start_frame = int(elements[2])
            end_frame = int(elements[3])
        ...
        multi_label = np.mean(np.asarray([len(x) for x in tmp])) > 5.0
        file_list = []  
        for item in tmp: 
            if self.test_mode:
                file_list.append([item[0], item[1], int(item[2]), int(item[3]), -1])
            else:
                labels = []
                ...
                file_list.append([item[0], item[1], int(item[2]), int(item[3]), labels])

        video_list = [VideoRecord(item[0], item[1], item[2], item[3], item[4]) for item in file_list]
    ...
    def __getitem__(self, index):
        ...
        landmarks = self.get_landmark(record, indices)
    ...
    def load_video_frames(self, video_path, frame_ids, retry=10, cvt=False):
            ...
            while ret:
                ...
                if ret:
                    if idx in frame_ids:
                        ...
                        if cvt:
                            frame_dict[str(idx)] = frame[..., ::-1]
                        else:
                            frame_dict[str(idx)] = frame
            capture.release()
            frames=[]
            for i in range(len(frame_ids)):
                frames.append(frame_dict[str(frame_ids[i])])
            return frames
    
    def get_crop_box(self, shape, box, scale=0.5):
        height, width = shape
    ...
    def process_single(self, tfm, image, d, h, w, crop_size):
        ...
        transformed_image = cv2.warpAffine(
            new_image, tfm, (crop_size, crop_size)
        )
        return transformed_image
    
    def crop_align_func(self, clip_frames, clip_bboxes, clip_landmarks_5, crop_size):
        shape = clip_frames[0].shape[:2] 
        big_bboxes=[]
        images=[]
        for i in range(len(clip_frames)):
            big_bbox=self.get_crop_box(shape, clip_bboxes[i], scale=0.5)
            x1, y1, x2, y2 = big_bbox
            cropped = clip_frames[i][y1:y2, x1:x2]
            big_bboxes.append(big_bbox)
            images.append(cropped)
        big_bboxes = np.array(big_bboxes)
        left_top = big_bboxes[:, :2].min(0) 
        right_bottom = big_bboxes[:, 2:].max(0)
        size = right_bottom - left_top
        w, h = size  
        clip_landmarks_5 = np.array(clip_landmarks_5).reshape(-1,5,2)
        new_five_landmarks = clip_landmarks_5 - left_top[None, ...] 
        landmark_for_estimiate = new_five_landmarks.copy()
        std_points = std_points_256 * crop_size / 256.0
        tfm, trans = estimiate_batch_transform(
            landmark_for_estimiate, tgt_pts=std_points
        )  
        diff = big_bboxes[:, :2] - left_top[None, ...] 
        if images is not None:
            transformed_images = [
                self.process_single(tfm, image, d, h, w, crop_size)
    ...
    def get_video_data(self, record, indices):
        crop_size = 112  
        images = []
        num_clips = self.num_clips 
        seq = []
        for seg_ind in indices:
            new_seg_ind = min(seg_ind + record.start_frame - 1 , record.end_frame) 
        video_path = os.path.join(self.root_path, record.path)
        video_frames = self.load_video_frames(video_path,seq)  
        alignment_file_path = os.path.join(self.root_path, record.align_file_path)
        alignment = np.loadtxt(alignment_file_path)
        frame_idx = alignment[:,0]
        bboxes = alignment[:,1:5]
        landmarks_5 = alignment[:,5:15]
        video_bboxes=[bboxes[int(np.where(frame_idx == frame)[0])] for frame in seq]
        video_landmarks_5=[landmarks_5[int(np.where(frame_idx == frame)[0])] for frame in seq]
        aligned_clips=[]
        for i in range(0, len(video_frames), self.num_frames):
            clip_frames = video_frames[i:i+4]
            clip_bboxes = video_bboxes[i:i+4]
            clip_landmarks_5 = video_landmarks_5[i:i+4]
            images_align = self.crop_align_func(clip_frames, clip_bboxes, clip_landmarks_5, crop_size)
            aligned_clips.append(images_align)
        aligned_clips = np.concatenate(aligned_clips, axis=0)
        for i in range(len(aligned_clips)):
            image = aligned_clips[i]
            image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
            images.append(image)
        return images
    
    def get_landmark(self, record, indices):
        landmark_path = os.path.join(self.root_path, record.landmark_file_path)
        video_landmarks = np.loadtxt(landmark_path) 
        landmarks=[]
        for i in range(0, len(indices), self.num_frames):  
            start = indices[i] - 1 + record.start_frame -1
            end = indices[i+3] + 2 + record.start_frame -1
            clip_landmark = video_landmarks[start:end].reshape((self.num_frames*self.sample_freq, -1, 2)) 
            landmarks.append(clip_landmark)  
        landmarks = np.stack(landmarks)  
        landmarks = torch.from_numpy(landmarks).permute(0, 3, 1, 2).contiguous()
        return landmarks
