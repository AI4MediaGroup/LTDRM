...
import math
from my_models.graph import Graph
from my_models.modules import ResGCN_Module
...
# Thanks to yfsong0709 for the released code on https://gitee.com/yfsong0709/ResGCNv1
...
class LTDRM(nn.Module):
    def __init__(self, **kwargs):
        super(LTDRM, self).__init__()
        ...
        # input branches
        self.input_branches = ResGCN_Input_Branch(structure, block, 2, A, **kwargs)

        # main stream
        module_list = [ResGCN_Module(32, 128, block, A, stride=2, **kwargs)]
        module_list += [ResGCN_Module(128, 128, block, A, **kwargs) for _ in range(structure[2] - 1)]
        module_list += [ResGCN_Module(128, 256, block, A, stride=2, **kwargs)]
        module_list += [ResGCN_Module(256, 256, block, A, **kwargs) for _ in range(structure[3] - 1)]
    ...
    def forward(self, x):
        ...
        # output
        x = self.global_pooling(x).squeeze(3).squeeze(2)  
        return x
...
#            trunc_normal_(self.absolute_pos_embed, std=.02)
        ...
        model_kwargs = dict(reduction = 4)
        self.landmark_mode = LTDRM(**model_kwargs)

        self.head = nn.Linear(self.num_features + 256, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def init_param(self):
        for m in self.landmark_mode.modules():
            if isinstance(m, nn.Conv1d) or isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                #m.bias = None
                if m.bias is not None:
...
#           x = x + self.absolute_pos_embed
    ...
    def forward(self, x):
        x1 = self.forward_features(x[0])  
        x2 = self.landmark_mode(x[1])  
...
# Thanks to rainy-xu for the released code on https://github.com/rainy-xu/TALL4Deepfake

@register_model
def TALL_SWIN_LTDRM(pretrained=False, **kwargs):
